...
function rprw_GetSelectedTraks(exclude_tracks, project)
    ...
    for i = 1, #exclude_tracks do
        exclude_track_numbers[i] = reaper.GetMediaTrackInfo_Value(exclude_tracks[i], "IP_TRACKNUMBER")
    end
    ...
    for i = reaper.CountSelectedTracks(project) - 1, 0, -1 do
        ...
        for i = 1, #exclude_track_numbers do
            if reaper.GetMediaTrackInfo_Value(track, "IP_TRACKNUMBER") == exclude_track_numbers[i] then
                exclude = true
            end
        end
        if not exclude then
...
function rprw_GetChildTracks(track, inclusive)
    local child_tracks = {}
    if inclusive then
        child_tracks[#child_tracks + 1] = track
    end
...
function rprw_SetExtState(section, key, value, persist)
    reaper.SetExtState(GetExtStateSectionName(section), key, value, persist or false)
end
...
function rprw_DeleteExtState(section, key, persist)
    return reaper.DeleteExtState(GetExtStateSectionName(section), key, persist or false)
end
...
function rprw_GetTransportExtState(key)
    return rprw_GetExtState("transport", key)
end
...
function rprw_HasTransportExtState(key)
    return rprw_HasExtState("transport", key)
end
