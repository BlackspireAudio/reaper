...
function ToggleMuteOnTrack(track, group, exclusive)
    ...
    if exclusive then
        other_traks_muted = AnyTrackMuted(reaper.IsTrackSelected(track), select(2, GetTrackId(track)))
        if other_traks_muted then
            rprw_UnmuteAllTracks()
        end
    end
...
function ToggleSoloOnTrack(track, group, exclusive, in_place)
    ...
    if exclusive then
        other_traks_soloed = AnyTrackSoloed(reaper.IsTrackSelected(track), select(2, GetTrackId(track)))
        if other_traks_soloed then
            rprw_UnsoloAllTracks()
        end
    end
    ...
    elseif solo_state == 1 then -- solo not-in-place
        rprw_SetTrackUISolo(track, in_place or other_traks_soloed, in_place, group)
    elseif solo_state == 2 then -- solo in-place
        rprw_SetTrackUISolo(track, not in_place or other_traks_soloed, in_place, group)
...
function ToggleSWSSoloMuteSlot(slot_id, save_on_reset, solo_mute, selected)
    if (solo_mute > 1 or AnyTrackSoloed()) and (solo_mute == 1 or AnyTrackMuted()) then
        if save_on_reset then rprw_StoreRecallSWSSoloMuteSlot(slot_id, solo_mute, selected, true) end
...
function AnyTrackPredicate(predicate, ignore_selected, ignore_track_id)
    ...
    for i = 0, reaper.CountTracks(0) - 1 do
        ...
        if not (
                i == ignore_track_id                                   -- ignore track with given index, always false if ignore_track_id <= -2 (default -2)
...
function CycleTrackRecMonStates(track, cycle_states)
    ...
    for i = 1, #cycle_states do
        if rec_arm == cycle_states[i].rec_arm and rec_mode == cycle_states[i].rec_mode and rec_mon == cycle_states[i].rec_mon then
            local next_state = cycle_states[i % #cycle_states + 1]
            rprw_SetTrackArmModeMonStates(track, next_state.rec_arm, next_state.rec_mode, next_state.rec_mon)
            return next_state
        end
    end
