...
function RestartPlayRecord(record, force_pre_roll, save_recorded_media)
    ...
    local current_loop_iteration    = tonumber(reaper.GetExtState(0, "blackspire_current_loop_iteration")) or 0
    local toggle_pre_roll_action_id = record and 41819 or 41818 -- Toggle playback pre-roll state
    local stop_action_id

    if current_loop_iteration > 0 or save_recorded_media == 1 then
    ...
    -- record restart is too fast for loop iteration counter to reset, so reset it manually
