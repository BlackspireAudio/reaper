...
function loop()
    if reaper.GetToggleCommandState(1068) == 1 then
        ...
        if current_playstate & 4 == 4 then
            ...
            if current_play_position < previous_play_position then
                if rprw_GetTransportExtState(recording_restarted_ext_state_key) == "true" then
                    ...
                    rprw_SetTransportExtState(loop_iteration_ext_state_key,
                        (tonumber(rprw_GetTransportExtState(loop_iteration_ext_state_key)) or 0) + 1)
                end
            end
    ...
    if terminate_on_record_stop and current_playstate & 4 == 0 then
        return
    end
    reaper.defer(loop)
end
