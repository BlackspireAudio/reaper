...
for i = 1, #selected_tracks do
    ...
    if rsw.IsFolderTrack(track) then
        ...
        local children = rsw.GetChildTracks(track, false, 1)
        for j = 1, #children do
            local child = children[j]
            if tm.HasSWSNoteTrait(child, tm.SWSNoteTrait.UmnuteOnUnfreeze) then
                tm.SetSWSNoteTrait(child, tm.SWSNoteTrait.UmnuteOnUnfreeze, false)
                rsw.SetTrackUIMute(child, false)
            end
        end
    end
end
