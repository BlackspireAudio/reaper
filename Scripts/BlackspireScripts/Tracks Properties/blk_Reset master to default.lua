-- @description Reset Master track hardware sends, channel count and volume to default
-- @version 1.0
-- @author Blackspire
...
--------------------PARAMS------------------------
--------------------------------------------------
local undo_message = "Reset Master track to default"
...
tm.DeleteAllTrackRoutings(master_track, 1) -- delete all hardware sends
local routing_idx = tm.CreateTrackRouting(master_track, nil)
tm.SetParam(master_track, tm.TrackParam.N_CHAN, 2)
tm.SetVolume(master_track, 1)
-- tm.SetRoutingChannels(master_track, routing_idx, 1)
