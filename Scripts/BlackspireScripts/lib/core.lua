...
-- @author Blackspire
-- @noindex
blackspire_lib_version = 1.0
--------------------------------------------------
function BSLoadLibraries(version, required_libs)
    local info = debug.getinfo(1, 'S');
    local script_path = info.source:match([[^@?(.*[\/])[^\/]-$]])

    if not version or version > blackspire_lib_version then
        reaper.MB(
            'Update ' .. script_path:gsub('%\\', '/') .. ' to version ' ..
            version .. ' or newer', 'BlackspireScripts', 0)
        return false
    end
...
function BSIncrUsedCount()
    local cnt = reaper.GetExtState('BlackspireScripts', 'counttotal')
    if cnt == '' then cnt = 0 end
    cnt = tonumber(cnt)
    if not cnt then cnt = 0 end
    cnt = cnt + 1
    reaper.SetExtState('BlackspireScripts', 'counttotal', cnt, true)
end
