...
--     author = "Blackspire",
--     version = "1.0",
--     script_template_folder = { reaper.GetResourcePath(), "Scripts", "BlackspireScripts", "Track Properties" },
...
function sgm.TryLoadGuiLibs(libs)
    ...
    for i = 1, #libs do GUI.req(libs[i])() end
...
function sgm.GenerateSelectedScriptVariations(script_gen_specs, enabled_options)
    ...
    if not loaded then
        reaper.MB(
            "Couldn't auto load all generated scripts. This means, that you will need to manually load the generated scripts using the action list 'load ReaScript' option",
            "Ignorable Whoops!", 0)
    end
