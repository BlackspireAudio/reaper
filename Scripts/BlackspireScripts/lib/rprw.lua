...
function rprw_GetSelectedTraks(exclude_tracks, project)
    ...
    for i = reaper.CountSelectedTracks(project) - 1, 0, -1 do
        ...
        for i = 1, #exclude_track_numbers do
            if reaper.GetMediaTrackInfo_Value(track, "IP_TRACKNUMBER") ==
                exclude_track_numbers[i] then exclude = true end
        end
...
function rprw_GetChildTracks(track, inclusive)
    local child_tracks = {}
...
function rprw_GetSelectedItems(project)
    project = project or 0
...
function rprw_SelectTracks(tracks)
    for i = 1, #tracks do
        if tracks[i] then reaper.SetTrackSelected(tracks[i], true) end
    end
end

function rprw_SelectItems(items)
...
function rprw_UnmuteAllTracks() reaper.Main_OnCommand(40339, 0) end

function rprw_UnsoloAllTracks() reaper.Main_OnCommand(40340, 0) end

function rprw_SelectTrackUnterMouse() reaper.Main_OnCommand(41110, 0) end

function rprw_GroupSelectedItems() reaper.Main_OnCommand(40032, 0) end
...
function rprw_SetTrackArmModeMonStates(track, rec_arm, rec_mode, rec_mon)
    ...
    if rprw_HasInstrumentFX(track) and
...
function rprw_UnselectAllMediaItems() reaper.Main_OnCommand(40289, 0) end
...
function rprw_SetExtState(section, key, value, persist)
    reaper.SetExtState(GetExtStateSectionName(section), key, value,
                       persist or false)
end

function rprw_GetExtState(section, key)
    return reaper.GetExtState(GetExtStateSectionName(section), key)
end

function rprw_DeleteExtState(section, key, persist)
    return reaper.DeleteExtState(GetExtStateSectionName(section), key,
                                 persist or false)
end

function rprw_HasExtState(section, key)
    return reaper.HasExtState(GetExtStateSectionName(section), key)
end

function rprw_SetTransportExtState(key, value, persist)
    rprw_SetExtState("transport", key, value, persist)
end

function rprw_GetTransportExtState(key) return
    rprw_GetExtState("transport", key) end

function rprw_DeleteTransportExtState(key, persist)
    rprw_DeleteExtState("transport", key, persist)
end

function rprw_HasTransportExtState(key) return
    rprw_HasExtState("transport", key) end

function rprw_SetGlobalExtState(key, value, persist)
    rprw_SetExtState("global", key, value, persist)
end

function rprw_GetGlobalExtState(key) return rprw_GetExtState("global", key) end

function rprw_DeleteGlobalExtState(key, persist)
    rprw_DeleteExtState("global", key, persist)
end

function rprw_HasGlobalExtState(key) return rprw_HasExtState("global", key) end

function rprw_SetMidiExtState(key, value, persist)
    rprw_SetExtState("midi", key, value, persist)
end

function rprw_GetMidiExtState(key) return rprw_GetExtState("midi", key) end

function rprw_DeleteMidiExtState(key, persist)
    rprw_DeleteExtState("midi", key, persist)
end

function rprw_HasMidiExtState(key) return rprw_HasExtState("midi", key) end
